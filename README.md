...
`run_stem_studio.bat` 会先激活 `D:\conda` 环境，再启动 `yyw.exe`。如果你使用的是便携 Python 运行时，可以改用：
...
如果使用 `run_stem_studio.bat`，确认 `D:\conda` 存在且其中安装了 Demucs。
...
`stem_studio.py` 是早期实现，当前主入口是 Rust 版 `yyw.exe`。除非需要对照旧逻辑，否则建议使用 Rust 版。
